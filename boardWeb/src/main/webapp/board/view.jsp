...
<%
	Member loginCheck = (Member)session.getAttribute("loginUser");
	...
	try{
		...
		String sql = " select * from board1 where bidx="+bidx;
...
<body>
	...
	<section>
		...
		<article>
			...
			<%
				...
				if(loginCheck != null && loginCheck.getMidx() == midx_){
			%>
					...
					<tbody>
						<% for(Reply r : rList){ %>
								<tr>
									...
									<% if(loginCheck != null && (loginCheck.getMidx() == r.getMidx())){ %>
									<td>
	...
	<script>
		...
		function goModify(){
			...
			if(loginCheck != 'null'){
					location.href = 'modify.jsp?bidx=<%=bidx%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>';
			}
			...
			if(loginCheck == 'null'){
				...
				$.ajax({
					...
					success: function(data){
						...
						var html = "";
						html +="<tr>";
						html +="<td>"+json[0].membername+"<input type='hidden' nmae='ridx' value='"+json[0].ridx+"'></td>";
		...
		function modifyFn(obj){
			...
			$(obj).parent().prev().html("<input type='text' name='rcontent' value='"+rcontent+"'><input type='hidden' name='orgin' value='"+rcontent+"'>");
		...
		function cancleReply(obj){
			...
			var html = "";
				html += "<input type='button' value='수정' onclick='updateReply(this)'>";
				html += "<input type='button' value='삭제' onclick='deleteFn(this)'>";
		...
		function updateReply(obj){
			...
			$.ajax({
				...
				success : function(data){
					...
					// 만약 수정 저장 후 수정, 삭제 버튼으로 복구 할 때 자신이 쓴글인지 비교가 필요하
